...
import com.kumhomarine.chartplotter.data.models.SavedPoint as DataSavedPoint
...
import com.kumhomarine.chartplotter.domain.usecases.*
...
data class MapUiState(
    ...
    val aisVisible: Boolean = true // AIS 표시 ON/OFF
)
...
class MainViewModel(
    ...
    fun updateAisVisible(visible: Boolean) {
        mapUiState = mapUiState.copy(aisVisible = visible)
    }
    ...
    fun updateShowPointEditSelectionDialog(show: Boolean) {
        dialogUiState = dialogUiState.copy(showPointEditSelectionDialog = show)
    }
    ...
    fun updateShowRouteSelectionForNavDialog(show: Boolean) {
        dialogUiState = dialogUiState.copy(showRouteSelectionForNavDialog = show)
    }
    ...
    fun updateShowTrackCreateDialog(show: Boolean) {
        dialogUiState = dialogUiState.copy(showTrackCreateDialog = show)
    }
    ...
    fun updateMapRotation(
        ...
        mapRotationUseCase.execute(
            ...
            coursePoint = mapUiState.coursePoint, // com.kumhomarine.chartplotter.SavedPoint 타입 그대로 전달
        ...
        map?.let { mapInstance ->
            ...
            if (closestPoint != null) {
                ...
                if (screenDistance <= 40) {
                    // 포인트 편집/삭제 다이얼로그 표시
                    // DataSavedPoint를 com.kumhomarine.chartplotter.SavedPoint로 변환
