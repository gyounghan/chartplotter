package com.kumhomarine.chartplotter.presentation.modules.chart.overlays
...
class AISOverlay {
    ...
    private val courseExtensionSourceId = "ais-course-extension-source"
    private val courseExtensionLayerId = "ais-course-extension-layer"
    ...
    private var geoJsonCourseExtensionSource: GeoJsonSource? = null
    private var courseExtensionLayer: LineLayer? = null
    ...
    fun stop() {
        mapRef?.getStyle { style ->
            try {
                if (style.getSource(courseExtensionSourceId) != null) style.removeSource(courseExtensionSourceId)
                if (style.getLayer(courseExtensionLayerId) != null) style.removeLayer(courseExtensionLayerId)
        ...
        geoJsonSource = null
        geoJsonCourseExtensionSource = null
        courseExtensionLayer = null
        circleLayer = null
        triangleLayer = null
     ...
     */
    /** bearing(도)와 거리(미터)로 목표 지점 좌표를 계산 */
    private fun destinationPoint(lat: Double, lon: Double, bearingDeg: Double, distanceMeters: Double): DoubleArray {
        val R = 6371000.0
    ...
    fun updateVessels(vessels: List<AISVessel>, aisCourseExtensionMeters: Float = 0f) {
        try {
            ...
            // AIS 코스 연장선 업데이트
            if (aisCourseExtensionMeters > 0f && geoJsonCourseExtensionSource != null) {
                val featuresJson = validVessels.mapNotNull { vessel ->
                    val lat = vessel.displayLatitude ?: vessel.latitude ?: return@mapNotNull null
                    val lon = vessel.displayLongitude ?: vessel.longitude ?: return@mapNotNull null
                    val end = destinationPoint(lat, lon, vessel.course.toDouble(), aisCourseExtensionMeters.toDouble())
                    val lat2 = end[0]
                    val lon2 = end[1]
                    """{"type":"Feature","geometry":{"type":"LineString","coordinates":[[$lon,$lat],[$lon2,$lat2]]}}"""
                }
                val geoJson = if (featuresJson.isEmpty()) {
                    """{"type":"FeatureCollection","features":[]}"""
                } else {
                    """{"type":"FeatureCollection","features":[${featuresJson.joinToString(",")}]}"""
                }
                geoJsonCourseExtensionSource?.setGeoJson(geoJson)
            } else if (geoJsonCourseExtensionSource != null) {
                geoJsonCourseExtensionSource?.setGeoJson("""{"type":"FeatureCollection","features":[]}""")
            }
        } catch (e: Exception) {
    ...
    private fun ensureStyleObjects(style: Style) {
        ...
        // stop() 비동기 완료 전 start() 재호출 시 기존 소스/레이어가 스타일에 남아있을 수 있음 → 먼저 제거
        if (style.getSource(courseExtensionSourceId) != null) {
            try {
                if (style.getLayer(courseExtensionLayerId) != null) style.removeLayer(courseExtensionLayerId)
                style.removeSource(courseExtensionSourceId)
            } catch (e: Exception) {
                Log.w("[AISOverlay]", "코스 연장 소스/레이어 제거 중 오류: ${e.message}")
            }
            geoJsonCourseExtensionSource = null
            courseExtensionLayer = null
        }
        if (style.getSource(sourceId) != null) {
            try {
                listOf(circleLayerId, watchlistLayerId, triangleLayerId, labelLayerId).forEach { layerId ->
                    if (style.getLayer(layerId) != null) style.removeLayer(layerId)
                }
                style.removeSource(sourceId)
                Log.d("[AISOverlay]", "기존 AIS 소스/레이어 제거 완료 (재시작 준비)")
            } catch (e: Exception) {
                Log.w("[AISOverlay]", "기존 소스/레이어 제거 중 오류: ${e.message}")
            }
            geoJsonSource = null
            circleLayer = null
            triangleLayer = null
        ...
        if (triangleLayer == null) {
            ...
            addLayerBelowShip(style, triangleLayer!!)
        ...
        if (watchlistTriangleLayer == null) {
            ...
            addLayerBelowShip(style, watchlistTriangleLayer!!)
            Log.d("[AISOverlay]", "WatchlistTriangleLayer 추가 완료: $watchlistLayerId")
        }

        // AIS 코스 연장선 소스 및 레이어
        if (geoJsonCourseExtensionSource == null) {
            geoJsonCourseExtensionSource = GeoJsonSource(courseExtensionSourceId, FeatureCollection.fromFeatures(emptyList())).also {
                style.addSource(it)
            }
            courseExtensionLayer = LineLayer(courseExtensionLayerId, courseExtensionSourceId).apply {
                setProperties(
                    PropertyFactory.lineColor(Color.rgb(0, 150, 0)),
            ...
            style.addLayerBelow(courseExtensionLayer!!, triangleLayerId)
            Log.d("[AISOverlay]", "AIS 코스 연장선 레이어 추가 완료")
        }
