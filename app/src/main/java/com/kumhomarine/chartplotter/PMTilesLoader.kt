...
object PMTilesLoader {
    ...
    /** 내 위치(선박) 레이어 아래에 추가 - 선박이 항상 맨 위에 보이도록 */
    private fun addLayerBelowShip(style: Style, layer: Layer) {
        try {
            if (style.getLayer("ship-symbol") != null) {
                style.addLayerBelow(layer, "ship-symbol")
            } else {
    ...
    /**
     * "숫자 지질" 형식에서 공백 이후 지질만 표시하는 표현식 생성
     * 예: "0 모래" → "모래", "0 모래, 0 진흙, 0 암석" → "모래, 진흙, 암석"
     * MapLibre slice, indexOf, concat 사용
     */
    private fun createStripNumericPrefixExpression(fieldName: String): Expression {
        return try {
            val getField = get(fieldName)
            // 세그먼트별 "N 지질" → "지질" 변환 (공백 이후만 추출)
            fun stripSegment(seg: Expression): Expression = switchCase(
                gte(indexOf(literal(" "), seg), literal(0)),
                slice(seg, sum(indexOf(literal(" "), seg), literal(1))),
                seg
            )
            val commaSep = literal(", ")
            val comma1 = indexOf(commaSep, getField)
            val comma2 = indexOf(commaSep, getField, sum(comma1, literal(2)))
            val part1 = slice(getField, literal(0), comma1)
            val part2 = slice(getField, sum(comma1, literal(2)), comma2)
            val part3 = slice(getField, sum(comma2, literal(2)))
            // 1개: 전체 strip | 2개: part1, part2 | 3개+: part1, part2, part3
            switchCase(
                lt(comma1, literal(0)),
                stripSegment(getField),
                lt(comma2, literal(0)),
                concat(stripSegment(part1), commaSep, stripSegment(slice(getField, sum(comma1, literal(2))))),
                concat(stripSegment(part1), commaSep, stripSegment(part2), commaSep, stripSegment(part3))
            )
        } catch (e: Exception) {
            Log.w("[PMTilesLoader]", "stripNumericPrefix 표현식 실패, 원본 사용: ${e.message}")
            get(fieldName)
        }
    }
    ...
    private fun addTextLayer(style: Style, config: PMTilesConfig, fontSize: Float = 14f) {
        ...
        // 텍스트 표현식: stripNumericPrefix 시 "숫자 지질" → 지질만 표시 (공백 기준 slice)
        val textFieldExpr = if (config.stripNumericPrefixFromTextField) {
            createStripNumericPrefixExpression(config.textField)
        } else {
        ...
        val symbolLayer = SymbolLayer("${config.sourceName}-labels", config.sourceName).apply {
            ...
            setProperties(
                // 텍스트 필드 설정
                PropertyFactory.textField(textFieldExpr),
...
//            setFilter(all(has(config.textField), gt(toNumber(get(config.textField)), literal(0))))
    ...
    fun addNavigationRoute(map: MapLibreMap, currentLocation: LatLng, waypoints: List<LatLng>, destination: LatLng) {
        map.getStyle { style ->
            try {
                ...
                addLayerBelowShip(style, navigationLineLayer)
    ...
    private fun addNavigationArrows(style: Style, routePoints: List<LatLng>) {
        try {
            ...
            addLayerBelowShip(style, arrowLayer)
    ...
    fun addRouteLine(map: MapLibreMap, routeId: String, points: List<org.maplibre.android.geometry.LatLng>, color: Int = android.graphics.Color.GREEN) {
        map.getStyle { style ->
            try {
                ...
                addLayerBelowShip(style, routeLineLayer)
    ...
    fun addRoutePoints(map: MapLibreMap, routeId: String, points: List<org.maplibre.android.geometry.LatLng>) {
        map.getStyle { style ->
            try {
                ...
                addLayerBelowShip(style, pointLayer)
    ...
    private const val GRID_SOURCE_ID = "grid_line_source"
    private const val GRID_LAYER_ID = "grid_line_layer"

    /**
     * 위경도선(격자선)을 지도에 추가합니다.
     * @param map MapLibreMap
     * @param intervalDegrees 격자 간격 (도 단위, 기본 0.5)
     */
    fun addGridLines(map: MapLibreMap, intervalDegrees: Double = 0.5) {
        map.getStyle { style ->
            try {
                if (style.getLayer(GRID_LAYER_ID) != null) style.removeLayer(GRID_LAYER_ID)
                if (style.getSource(GRID_SOURCE_ID) != null) style.removeSource(GRID_SOURCE_ID)
                val minLat = 20.0
                val maxLat = 55.0
                val minLon = 115.0
                val maxLon = 150.0
                val features = mutableListOf<String>()
                var lat = (minLat / intervalDegrees).toInt() * intervalDegrees
                while (lat <= maxLat) {
                    features.add("""
                        {
                            "type": "Feature",
                ...
                var lon = (minLon / intervalDegrees).toInt() * intervalDegrees
                while (lon <= maxLon) {
                    features.add("""
                        {
                            "type": "Feature",
                ...
                val source = GeoJsonSource(GRID_SOURCE_ID, geoJson)
                style.addSource(source)
                val layer = LineLayer(GRID_LAYER_ID, GRID_SOURCE_ID)
                    .withProperties(
                        PropertyFactory.lineColor(Color.rgb(100, 100, 100)),
                        PropertyFactory.lineWidth(1.0f)
                    )
                addLayerBelowShip(style, layer)
                Log.d("[PMTilesLoader]", "위경도선 추가됨: 간격 ${intervalDegrees}°")
            } catch (e: Exception) {
                Log.e("[PMTilesLoader]", "위경도선 추가 실패: ${e.message}")
            }
        }
    }
     ...
     * 위경도선을 제거합니다.
     */
    fun removeGridLines(map: MapLibreMap) {
        map.getStyle { style ->
            try {
                if (style.getLayer(GRID_LAYER_ID) != null) style.removeLayer(GRID_LAYER_ID)
                if (style.getSource(GRID_SOURCE_ID) != null) style.removeSource(GRID_SOURCE_ID)
    ...
    fun addNavigationMarker(map: MapLibreMap, location: LatLng, name: String) {
        map.getStyle { style ->
            try {
                ...
                addLayerBelowShip(style, navigationMarkerLayer)
    ...
    private const val MAP_HIDDEN_LAYER_ID = "map_hidden_fill"
    private const val MAP_HIDDEN_SOURCE_ID = "map_hidden_source"
    private const val DISTANCE_CIRCLE_SOURCE_ID = "distance_circle_source"
    private const val DISTANCE_CIRCLE_LAYER_ID = "distance_circle_layer"

    /**
     * 지도 감춤: 배경 지도 위에 불투명 레이어를 덮어 지도를 숨깁니다.
     */
    fun addMapHiddenOverlay(map: MapLibreMap) {
        map.getStyle { style ->
            try {
                if (style.getLayer(MAP_HIDDEN_LAYER_ID) != null) return@getStyle
                val worldPolygon = """
                {
                    "type": "FeatureCollection",
                    "features": [{
                ...
                """.trimIndent()
                val source = GeoJsonSource(MAP_HIDDEN_SOURCE_ID, worldPolygon)
                style.addSource(source)
                val layer = FillLayer(MAP_HIDDEN_LAYER_ID, MAP_HIDDEN_SOURCE_ID)
                    .withProperties(
                        PropertyFactory.fillColor(Color.rgb(50, 50, 50))
                    )
                try {
                    style.addLayerBelow(layer, "ship-symbol")
                } catch (_: Exception) {
                ...
                Log.e("[PMTilesLoader]", "지도 감춤 레이어 추가 실패: ${e.message}")
            }
        }
    }

    fun removeMapHiddenOverlay(map: MapLibreMap) {
        map.getStyle { style ->
            try {
                if (style.getLayer(MAP_HIDDEN_LAYER_ID) != null) style.removeLayer(MAP_HIDDEN_LAYER_ID)
                if (style.getSource(MAP_HIDDEN_SOURCE_ID) != null) style.removeSource(MAP_HIDDEN_SOURCE_ID)
            } catch (_: Exception) { }
        }
    }

    /**
     * 거리원: 선박 위치 중심 반경(미터) 원을 표시합니다.
     * 소스가 이미 있으면 GeoJSON만 갱신하여 "already exists" 충돌을 방지합니다.
     */
    fun updateDistanceCircle(map: MapLibreMap, center: LatLng, radiusMeters: Float) {
        val lat = center.latitude
        val lon = center.longitude
        val metersPerDegreeLat = 111320.0
        val metersPerDegreeLon = 111320.0 * kotlin.math.cos(Math.toRadians(lat))
        val deltaLat = (radiusMeters / metersPerDegreeLat)
        val deltaLon = (radiusMeters / metersPerDegreeLon)
        val coords = (0..36).map { i ->
            val angle = Math.toRadians(i * 10.0)
            val dy = deltaLat * kotlin.math.cos(angle)
            val dx = deltaLon * kotlin.math.sin(angle)
            "[${lon + dx}, ${lat + dy}]"
        }.joinToString(",")
        val geoJson = """{"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"Polygon","coordinates":[[$coords]]}}]}"""
        map.getStyle { style ->
            try {
                val existingSource = style.getSource(DISTANCE_CIRCLE_SOURCE_ID)
                if (existingSource is GeoJsonSource) {
                    existingSource.setGeoJson(geoJson)
                } else if (existingSource == null) {
                    style.addSource(GeoJsonSource(DISTANCE_CIRCLE_SOURCE_ID, geoJson))
                    val layer = FillLayer(DISTANCE_CIRCLE_LAYER_ID, DISTANCE_CIRCLE_SOURCE_ID)
                        .withProperties(
                            PropertyFactory.fillColor(Color.argb(40, 0, 100, 255)),
                            PropertyFactory.fillOutlineColor(Color.argb(180, 0, 100, 255))
                        )
                    try {
                        style.addLayerBelow(layer, "ship-symbol")
                    } catch (_: Exception) {
                ...
                Log.e("[PMTilesLoader]", "거리원 업데이트 실패: ${e.message}")
            }
        }
    }

    fun removeDistanceCircle(map: MapLibreMap) {
        map.getStyle { style ->
            try {
                if (style.getLayer(DISTANCE_CIRCLE_LAYER_ID) != null) style.removeLayer(DISTANCE_CIRCLE_LAYER_ID)
                if (style.getSource(DISTANCE_CIRCLE_SOURCE_ID) != null) style.removeSource(DISTANCE_CIRCLE_SOURCE_ID)
            } catch (_: Exception) { }
        }
    }

    private const val HEADING_LINE_SOURCE_ID = "heading_line_source"
    private const val HEADING_LINE_LAYER_ID = "heading_line_layer"

    /**
     * bearing(도)와 거리(미터)로 목표 지점 좌표를 계산합니다
     */
    private fun destinationPoint(lat: Double, lon: Double, bearingDeg: Double, distanceMeters: Double): LatLng {
    ...
    /**
     * 해딩연장: 선박 위치에서 heading 방향으로 extensionLength 미터만큼 연장선을 그립니다.
     * 소스가 이미 있으면 GeoJSON만 갱신하여 충돌을 방지합니다.
     */
    fun addHeadingLine(map: MapLibreMap, center: LatLng, headingDeg: Float, extensionLengthMeters: Float) {
        if (extensionLengthMeters <= 0f) return
        val end = destinationPoint(center.latitude, center.longitude, headingDeg.toDouble(), extensionLengthMeters.toDouble())
        val geoJson = """
        {"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"LineString","coordinates":[[${center.longitude},${center.latitude}],[${end.longitude},${end.latitude}]]}}]}
        """.trimIndent()
        map.getStyle { style ->
            try {
                val existingSource = style.getSource(HEADING_LINE_SOURCE_ID)
                if (existingSource is GeoJsonSource) {
                    existingSource.setGeoJson(geoJson)
                } else if (existingSource == null) {
                    style.addSource(GeoJsonSource(HEADING_LINE_SOURCE_ID, geoJson))
                    val layer = LineLayer(HEADING_LINE_LAYER_ID, HEADING_LINE_SOURCE_ID)
                        .withProperties(
                ...
                Log.e("[PMTilesLoader]", "해딩연장 선 추가 실패: ${e.message}")
            }
        }
    }

    fun removeHeadingLine(map: MapLibreMap) {
        map.getStyle { style ->
            try {
                if (style.getLayer(HEADING_LINE_LAYER_ID) != null) style.removeLayer(HEADING_LINE_LAYER_ID)
                if (style.getSource(HEADING_LINE_SOURCE_ID) != null) style.removeSource(HEADING_LINE_SOURCE_ID)
            } catch (_: Exception) { }
        }
    }
    ...
    fun addCourseLine(map: MapLibreMap, currentLocation: LatLng, destination: LatLng) {
        map.getStyle { style ->
            try {
                ...
                addLayerBelowShip(style, courseLineLayer)
        ...
        map.getStyle { style ->
            try {
                ...
                if (isHighlighted) {
                    ...
                    addLayerBelowShip(style, backgroundLayer)
                }
                ...
                addLayerBelowShip(style, trackLineLayer)
                }
                ...
                if (isHighlighted) {
                    ...
                    addLayerBelowShip(style, pointBackgroundLayer)
                }
                ...
                addLayerBelowShip(style, pointMarkerLayer)
